...
def open_npy_and_obj(name_to_open_):
    ...
    if not check_pcd_grasp_points:
        ...
        pcd_files = glob.glob("/media/nuno/Data/ycb-tools/models/real_data_Nuno/001/google_512k/complete_pc.pcd")
        obj_ = pcd_files
        obj_.sort()
...
def show_selected_grasps_with_color(m, ply_name_, title, obj_):
    m_good = m[m[:, -2] <= 0.6]		# second to last number is the metric score
    #print('m_good', m_good)
    ...
    if save_fig or show_fig:
        # fig 1: good grasps
        compl_pc = o3d.io.read_point_cloud('/media/nuno/Data/ycb-tools/models/real_data_Nuno/001/google_512k/complete_pc_x.pcd')
        xyz_load = np.asarray(compl_pc.points)
        x = xyz_load[:,0]
        y = xyz_load[:,1]
        z = xyz_load[:,2]
        _, _, _, value = np.random.random((4, len(x)))

        #value = 0.5*np.ones([len(x)])
...
if __name__ == "__main__":  
    npy_names = glob.glob("/media/nuno/Data/ycb-tools/models/real_data_Nuno/*.npy")
    ...
    for i in range(len(npy_names)):
        ...
        #print('grasps', grasps_with_score)
