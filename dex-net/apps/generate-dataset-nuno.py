...
os.makedirs(save_dir + "models/ycb_generated_output", exist_ok=True)
...
def do_job(i):
    ...
    if os.path.exists(str(file_list_all[i]) + "/google_512k/nontextured.obj"):
        good_grasp = worker(i, 10, 1, good_grasp)
        
        good_grasp_file_name = save_dir + "models/ycb_generated_output/" + "{}_{}_{}".format(filename_prefix, str(object_name), str(len(good_grasp)))
        ...
        object_name = file_list_all[i][len(os.environ["HOME"]) + 28:]
        print("Folder {} with no google mesh. Next!".format(object_name))
        return
...
def worker(i, sample_nums, grasp_amount, good_grasp):
    ...
    less_class = False  # less class can accelerate the dataset generate
    if less_class:
        fc_list = np.array([2.0, 1.6, 0.6]) # -1 0.6
    else:
